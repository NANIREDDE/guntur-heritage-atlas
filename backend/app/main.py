...
from .schemas import LocalityDetail, LocalitySummary, MediaSummary, SourceSummary, TempleDetail, TempleSummary
...
def source_summary(source: Source) -> SourceSummary:
    return SourceSummary(id=source.source_id, title=source.title, publisher=source.publisher, url=source.url, source_type=source.source_type)
...
def stats(db: Session = Depends(get_db)) -> dict[str, int]:
    historical_index = HISTORICAL_DIR / "GHA-HIST-1961-GUNTUR-TALUK-INDEX.json"
    historical_entries = 0
    if historical_index.exists():
        try:
            historical_entries = len(json.loads(historical_index.read_text(encoding="utf-8")).get("entries", []))
        except (OSError, json.JSONDecodeError):
            pass
    return {"temples": len(db.scalars(select(Temple)).all()), "localities": len(db.scalars(select(Locality)).all()), "sources": len(db.scalars(select(Source)).all()), "routes": len(list(ROUTE_DIR.glob("*.json"))), "historical_observations": historical_entries}
...
def list_temples(db: Session = Depends(get_db)) -> list[TempleSummary]:
    ...
    return [TempleSummary(id=t.temple_id, name=t.name, city=t.locality.name if t.locality else "Guntur", status=t.evidence_status) for t in temples]
...
def list_localities(db: Session = Depends(get_db)) -> list[LocalitySummary]:
    ...
    return [LocalitySummary(id=l.locality_id, name=l.name, type=l.locality_type, district=l.current_district, status=l.evidence_status) for l in localities]
...
def get_locality(locality_id: str, db: Session = Depends(get_db)) -> LocalityDetail:
    ...
    temples = [TempleSummary(id=t.temple_id, name=t.name, city=locality.name, status=t.evidence_status) for t in sorted(locality.temples, key=lambda item: item.name)]
    return LocalityDetail(id=locality.locality_id, name=locality.name, type=locality.locality_type, district=locality.current_district, status=locality.evidence_status, state=locality.state, country=locality.country, name_origin=locality.name_origin_summary, history=locality.history_summary, temples=temples, sources=[source_summary(source) for source in locality.sources])
